...
> 미국에선 Sora 1이 종료됐고, Sora 2가 기본값이 됐습니다.
...
**영상판엔 `Sora 1` 종료 공지가 떴고, 음악판엔 기능 폭격이 떨어졌고, LLM 진영은 백오피스 공사를 시작한 한 주였습니다.**

영상 생성 쪽에선 미국 기준 `Sora 1` 종료와 `Sora 2` 기본 전환이 공식화됐습니다. 제가 확인한 2026년 3월 25일 기준 공식 문서에는 `Sora 2` 철수 계획이 아니라, 반대로 `Sora 2`를 단일 기본 경험으로 밀겠다는 내용이 적혀 있습니다. 음악 생성 쪽에선 Suno가 입력, 리스타일링, 스타일 고정 기능을 한꺼번에 밀어 넣으면서 '한 곡 뽑기'보다 '내 작업물을 바로 이어 쓰기' 경쟁으로 판을 돌렸습니다. LLM 진영은 성능 숫자보다 도구 연결, 평가, 장기 실행 같은 실무 레이어를 두껍게 만드는 쪽으로 움직였습니다. 이미지 생성 쪽도 V8 Alpha와 Relax 모드처럼 결과물 자체보다 반복 실험 비용을 낮추는 방향이 눈에 띄었습니다.
...
**핵심만 말하면**
Anthropic가 Claude로 오래 도는 앱 개발을 굴릴 때 필요한 하네스 설계 경험을 풀었습니다.

**왜 중요하냐면**
이건 데모 성능보다 개발 공정, 평가, 장기 실행 같은 진짜 실무 레이어를 두껍게 만드는 변화입니다.
...
**한 줄로 요약하면**
Google가 TV 화면에서도 `Gemini` 답변을 더 길고 풍부하게 보여주는 기능을 붙였습니다.

**현업 기준으로 보면**
모델 자체보다도, AI 기능이 일상 기기 인터페이스 안으로 더 깊게 들어온다는 신호로 보는 편이 맞습니다.

**쉽게 비유하면**
AI를 폰 속 앱이 아니라 거실 리모컨 속 기본 기능으로 밀어 넣는 느낌입니다.
...
**이건 뭐가 바뀐 건데?**
Anthropic가 Claude 평가에서 벤치마크 오염과 정답 유출 문제를 어떻게 봐야 하는지 짚었습니다.
...
**이건 뭐가 바뀐 건데?**
Anthropic가 AI 에이전트 평가를 왜, 어떻게 설계해야 하는지 실전 기준으로 정리했습니다.
...
**핵심만 말하면**
Google가 `Gemini API`에 도구 조합, 컨텍스트 순환, 지도 그라운딩 같은 실전 기능을 한꺼번에 붙였습니다.
...
**한 줄로 요약하면**
Midjourney가 `V8 Alpha`에 저비용으로 많이 돌려볼 수 있는 `Relax mode`를 붙였습니다.

**왜 중요하냐면**
이미지판에선 결과물 한 장보다 반복 실험 비용과 속도가 달라져 손이 더 자주 가는 쪽이 중요합니다.

**한 장면으로 보면**
화질 자랑보다 필름값이 싸져서 테스트 컷을 훨씬 많이 찍게 되는 쪽에 가깝습니다.
...
**이건 뭐가 바뀐 건데?**
Midjourney가 차세대 이미지 모델 `V8 Alpha` 테스트를 본격적으로 열었습니다.
...
**핵심만 말하면**
OpenAI가 2026년 3월 13일부로 `Sora 1`을 미국에서 내리고, `Sora 2`를 기본 경험으로 돌렸습니다.

**진짜 중요한 건**
기존 버전에서 만든 자산이 있다면 내보내기 가능 기간, 호환성, 새 버전 적응 비용을 바로 체크해야 합니다.
...
**한 줄로 요약하면**
Suno가 내 오디오를 다른 결로 다시 입히는 `Covers` 베타를 공개했습니다.

**그래서 뭐가 달라지냐면**
이건 프롬프트 한 줄 경쟁보다, 내 소스와 스타일을 바로 가져다 쓰는 워크플로 경쟁으로 넘어갔다는 뜻입니다.
...
**이건 뭐가 바뀐 건데?**
Suno가 장면이나 사진 같은 시각 단서를 음악 출발점으로 쓰는 `Suno Scenes`를 내놨습니다.
...
**핵심만 말하면**
Suno가 사용자 업로드를 다루는 과정에 저작권 식별 장치를 붙였습니다.

**진짜 중요한 건**
생성 기능이 커질수록 저작권 검증과 업로드 안전장치가 같이 붙는 흐름도 더 강해집니다.
...
**한 줄로 요약하면**
Suno가 녹음본이나 오디오 파일을 바로 넣어 곡으로 확장하는 `Audio Inputs`를 열었습니다.
...
**쉽게 말하면**
Suno가 한 번 잡은 음악 스타일을 여러 곡에 이어 쓰게 해주는 `Personas`를 공개했습니다.
